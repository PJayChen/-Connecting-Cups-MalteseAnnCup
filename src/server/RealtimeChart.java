...
public class RealtimeChart {
	...
	public void updateChart(List<Integer> axis_x, List<FeatureVector> fvList) {
		List<Integer> accel_x = new LinkedList<Integer>(), accel_y = new LinkedList<Integer>(),
				accel_z = new LinkedList<Integer>(), accel_magnitude = new LinkedList<Integer>();
		for (FeatureVector fv:fvList) {
			accel_x.add(fv.getAcceleration_x());
			accel_y.add(fv.getAcceleration_y());
			accel_z.add(fv.getAcceleration_z());
			accel_magnitude.add(fv.getAcceleration_magnitude());
		}
		...
		chartPanel.updateSeries(RealtimeChart.SERIES_NAME_4, axis_x, accel_magnitude, null);
	}

	public void updateChart(List<Integer> accel_magnitude, List<Integer> accel_x, List<Integer> accel_y,
			List<Integer> accel_z, List<Integer> axis_x) {
		chartPanel.updateSeries(RealtimeChart.SERIES_NAME_1, axis_x, accel_x, null);
		chartPanel.updateSeries(RealtimeChart.SERIES_NAME_2, axis_x, accel_y, null);
		chartPanel.updateSeries(RealtimeChart.SERIES_NAME_3, axis_x, accel_z, null);
		chartPanel.updateSeries(RealtimeChart.SERIES_NAME_4, axis_x, accel_magnitude, null);
